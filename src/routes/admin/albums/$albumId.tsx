  ...
  if (!album) {
    return (
      <section className='flex min-w-0 flex-col'>
        ...
        <div className='grid place-items-center py-16 text-[#777a75] text-sm'>
          该专辑不存在或已被删除
        </div>
      </section>
    )
  ...
  return (
    <section className='flex min-w-0 flex-col'>
      <div className='flex min-h-14 items-center gap-3 border-[#333331] border-b bg-[#1d1e22] px-4'>
        ...
        {album.agency && (
          <span className='text-[#9da19b] text-sm'>{album.agency.name}</span>
        )}
        <span className='ml-auto text-[#9da19b] text-xs'>
      ...
      <div className='min-h-0 flex-1 overflow-y-auto p-4'>
        {imagesQuery.isLoading ? (
          ...
          <div className='grid grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-3'>
            {images.map((img) => (
              <div
                className='group relative overflow-hidden rounded-md border border-[#333331] bg-[#202125]'
                key={img.id}
              >
                {img.thumbnailUrl ? (
                  <img
                  ...
                  <div className='flex aspect-[3/4] w-full items-center justify-center bg-[#17181b] text-[#777a75] text-xs'>
                    无预览
                  </div>
                )}
                <div className='absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-2'>
                  <p className='truncate text-[#ebe7df] text-xs'>{img.title}</p>
                </div>
              </div>
            ))}
          </div>
