...
function TagsPage() {
  ...
  const [color, setColor] = useState('#73e0d3')
  ...
  const createMutation = useMutation({
    mutationFn: () => orpc.admin.tags.create.call({ color, name }),
    onSuccess: () => {
      setName('')
    ...
    <section className='flex min-w-0 flex-col'>
      ...
      <div className='min-h-0 flex-1 overflow-y-auto p-5'>
        <div className='mb-5 flex max-w-3xl flex-wrap items-center gap-2'>
          ...
          <input
            className='h-9 w-24 rounded-md border border-[#383a37] bg-[#17181b] px-2'
